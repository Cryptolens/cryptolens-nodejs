...
module.exports = class Helpers {
  ...
  static LoadFromString(rsaPubKey, string, signatureExpirationInterval = 0) {
    ...
    if (helpers.VerifySignature(response, rsaPubKey)) {
      var licenseKey = JSON.parse(
        Buffer.from(response.licenseKey, "base64").toString("utf-8")
      );
      var signed = new Date(licenseKey.SignDate * 1000);
      var exp = new Date(
        signed.getFullYear(),
        signed.getMonth(),
        signed.getDate() + signatureExpirationInterval
      );
  ...
  static GetMachineCode_beta() {
    if (process.platform === "win32") {
      return execSync("cmd.exe /C wmic csproduct get uuid", {
        encoding: "utf8",
      });
